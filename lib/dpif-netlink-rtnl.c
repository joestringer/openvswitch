...
static int
dpif_netlink_rtnl_gre_destroy(const char *name)
{
    return dpif_netlink_rtnl_destroy(name);
}

static int
dpif_netlink_rtnl_geneve_destroy(const char *name)
{
    return dpif_netlink_rtnl_destroy(name);
}
...
dpif_netlink_rtnl_gre_create_kind(struct netdev *netdev, const char *kind)
{
    ...
    if (!err && (err = dpif_netlink_rtnl_gre_verify(netdev, name, kind))) {
        dpif_netlink_rtnl_gre_destroy(name);
    }
...
dpif_netlink_rtnl_geneve_create_kind(struct netdev *netdev, const char *kind)
{
    ...
    if (!err && (err = dpif_netlink_rtnl_geneve_verify(netdev, name, kind))) {
        dpif_netlink_rtnl_geneve_destroy(name);
    }
...
dpif_netlink_rtnl_port_destroy(const char *name, const char *type)
{
    ...
    case OVS_VPORT_TYPE_GRE:
        return dpif_netlink_rtnl_gre_destroy(name);
    case OVS_VPORT_TYPE_GENEVE:
        return dpif_netlink_rtnl_geneve_destroy(name);
    case OVS_VPORT_TYPE_NETDEV:
    case OVS_VPORT_TYPE_INTERNAL:
    case OVS_VPORT_TYPE_LISP:
    case OVS_VPORT_TYPE_STT:
